...
Next.js загружает всё портфолио одним GraphQL-запросом на сервере, проверяет контракт ответа,
формирует HTML и кеширует результат по тегу `portfolio`. Защищённый endpoint ревалидации позволяет
обновить кеш после изменения профиля, не раскрывая адрес API и секрет в браузере.

Посетитель может отправить через форму имя, email, необязательное название компании и сообщение.
Server Action повторно валидирует поля и выполняет GraphQL mutation; API сохраняет обращение в
PostgreSQL. В браузер возвращается только безопасный статус успеха, ошибки валидации или общая
ошибка отправки.
